...
- Build an Inventory Management module allowing users to execute CRUD operations on medicine records with dynamic stock tracking and CSV export.
...
3. **Inventory Management Module:**
   ...
   - Includes quick-restock actions, deletion confirmation dialogs, and formats currency columns safely using JavaFX `CellFactory`.
...
3. **Portable Database Deployment:**
   ...
   - *Solution:* Migrated to an embedded SQLite database (`pharmacy.db`) via the `sqlite-jdbc` Maven dependency. The database file is auto-created alongside the application on first run, requiring zero external setup.
